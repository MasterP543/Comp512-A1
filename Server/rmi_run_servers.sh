...
tmux new-session \; \
  ...
  select-pane -t 0 \; \
  send-keys "ssh -t ${MACHINES[3]} \"cd $(pwd) > /dev/null; ./run_rmi.sh; echo -n 'Connected to '; hostname; sleep .5s; ./rmi_run_middleware.sh ${MACHINES[0]} ${MACHINES[1]} ${MACHINES[2]}\"" C-m \;